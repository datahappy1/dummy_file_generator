...
def list_to_stmt(statement, add_plus_sign):
    if bool(add_plus_sign):
...
def return_csv_header(column_list):
    return list_to_stmt(statement=column_list, add_plus_sign=False).replace(' ','') + ',\n'
...
def return_flat_header(column_name_list, column_len_list):
    ...
    return list_to_stmt(header_row, add_plus_sign=False).replace(', ', '') + '\n'
